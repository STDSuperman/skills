...
class IllustrationGenerator:
    ...
    def __init__(self, parsed_lyrics_file: Path, output_dir: Path, image_generator_script: Path):
    ...
    def generate_all(self) -> List[Dict[str, Any]]:
        ...
        print(f"Generating {len(sections)} illustrations for '{self.parsed_data['song_title']}'...\n")

        for i, section in enumerate(sections, 1):
            print(f"[{i}/{len(sections)}] Generating: {section['name']} ({section['id']})")
    ...
    def save_metadata(self, results: List[Dict[str, Any]]):
        ...
        metadata = {
            'song_title': self.parsed_data['song_title'],
            'sections': results
        }
...
def main():
    parser = argparse.ArgumentParser(description='Generate illustrations for parsed lyrics')
    parser.add_argument('parsed_lyrics', type=Path, help='Path to parsed lyrics JSON file')
    parser.add_argument('--output-dir', type=Path, default=Path('output/illustrations'),
                        help='Output directory for illustrations')
    parser.add_argument('--image-generator', type=Path,
    ...
    generator = IllustrationGenerator(args.parsed_lyrics, args.output_dir, args.image_generator)
